...
class EthernetFrame:
	...
	def __str__(self):
		ether = hex(self.EtherType)
		trans = "UNKNOWN"
	
		if self.EtherType in ETHER_TYPE_DICT:
			trans = ETHER_TYPE_DICT[self.EtherType]
		
		source = self.mac_to_str(self.Source)
		dest = self.mac_to_str(self.Destination)
		length = len(self.Payload)
	
		return f"[ Ethernet - {ether} {trans}; Source: {source}; Destination: {dest}; Len:{length} ]"
